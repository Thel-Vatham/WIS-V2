...
class ReasoningEngine:
    ...
    async def think(
        ...
        api_tools: List[Dict[str, Any]] = self._tools_to_api_format(tools) if tools else []

        raw_chunks: List[str] = []
        try:
            async for chunk in self.llm_client.stream(
                messages, tools=api_tools or None, model=model
            ):
    ...
    def _normalize_calls_namespace(self, calls: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        construido por _tools_to_api_format() y aseguramos que existan
    ...
    def _tools_to_api_format(self, tools: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Convierte el esquema interno de habilidades a formato API tools.

        Construye y actualiza self._tool_map para poder revertir los

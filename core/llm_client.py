...
Conecta con proveedores de LLM compatibles con API estandar (DeepSeek, OpenRouter, etc).
...
class LLMClient:
    """Cliente async para proveedores LLM compatibles con API estandar."""
    ...
    def __init__(
        ...
        *,
        ...
        self.api_key: str = api_key or os.environ.get("DEEPSEEK_API_KEY", "") or os.environ.get("API_KEY", "")
                ...
                async with client.stream(
                    ...
                    async for line in response.aiter_lines():
                        ...
                        if time.time() - last_data_time > 45.0:
                            logger.error("LLMClient: Timeout esperando el primer token real (API atascada en keep-alive).")
                            raise httpx.TimeoutException("Time to first token exceeded 45s due to infinite keep-alives.")
    ...
    async def chat(
        ...
        *,
        ...
        """Ejecuta una peticion de chat no-streaming y devuelve {'text': str, 'calls': list}."""
        full_text: List[str] = []
        tool_calls: List[Dict[str, Any]] = []

        async for chunk in self.stream(messages, tools=tools, model=model, temperature=temperature):
            try:
                data = json.loads(chunk)
                if isinstance(data, dict) and "calls" in data:
                    tool_calls.extend(data["calls"])
                    continue
            except (json.JSONDecodeError, TypeError):
                pass
            full_text.append(chunk)

        return {"text": "".join(full_text), "calls": tool_calls}

class ModelRouter:
    """Enruta entre un modelo rapido y uno fuerte (solo cloud)."""

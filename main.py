...
def build_core(settings: dict) -> dict:
    ...
    async def _on_telemetry_trigger(evt: dict) -> None:
        await praxis.handle_autonomous_trigger(evt)
...
def close_core(core: dict) -> None:
    ...
    lh = core.get("long_horizon")
    if lh and hasattr(lh, "stop"):
        try:
            lh.stop()
        except Exception as e:
            logger.warning(f"Error closing long_horizon: {e}")

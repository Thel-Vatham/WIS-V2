...
class AbilityRegistry:
    ...
    # Registro / consulta
    # ------------------------------------------------------------------ #
    def register(self, ability: Ability, overwrite: bool = False) -> bool:
        """
        Registers an ability. Returns True if registered successfully.

        Args:
            ability: The Ability instance to register.
            overwrite: If True, replaces an existing ability with the same name.
                       If False (default), skips registration and returns False.
        """
        if not isinstance(ability, Ability):
            logger.warning("register: object is not an Ability instance: %s", type(ability))
            return False
        name = ability.name
        if name in self._abilities:
            if overwrite:
                logger.info("register: overwriting existing ability '%s'", name)
                self._abilities[name] = ability
                return True
            else:
                logger.debug("register: ability '%s' already registered (use overwrite=True to replace)", name)
                return False
        self._abilities[name] = ability
        logger.debug("register: ability '%s' registered", name)
        return True
